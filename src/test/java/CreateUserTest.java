...
public class CreateUserTest {
    ...
    public void createUserWithoutEmail() {
        ...
        Response response = UsersMethods.createUser(user);
        int statusCode = response.then().extract().statusCode();
        Assert.assertEquals("При регистрации клиента, без поля emali, статус Код отличается от 403", 403, statusCode);
        String body = response.then().extract().jsonPath().getString("message");
        Assert.assertEquals("Тело ответа при создание,клиента без поля emali, отличается от ожидаемого", "Email, password and name are required fields", body);
    }

    @DisplayName("Проверка создания клиента без поля passwrod")
    @Test
    public void createUserWithoutPassword() {
        user = new User(faker.internet().emailAddress(), Optional.ofNullable(null), String.valueOf(faker.name()));
        Response response = UsersMethods.createUser(user);
        int statusCode = response.then().extract().statusCode();
        Assert.assertEquals("При регистрации клиента, без поля passwrod, статус Код отличается от 403", 403, statusCode);
        String body = response.then().extract().jsonPath().getString("message");
        Assert.assertEquals("Тело ответа при создание,клиента без поля passwrod, отличается от ожидаемого", "Email, password and name are required fields", body);
    }
